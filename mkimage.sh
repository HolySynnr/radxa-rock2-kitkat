...
U_BOOT_BIN="$(basename u-boot/RK3288*boot*.bin)"
export U_BOOT_BIN
...
if [ ! -d "rockdev" ]; then
ln -s RKTools/linux/Linux_Upgrade_Tool_v1.2/rockdev rockdev
fi
...
then
	...
	cp -va $OUT/recovery.img $IMAGE_PATH/
	echo "done."
else
	...
	cp -va rkst/Image/misc.img $IMAGE_PATH/misc.img
	cp -va rkst/Image/pcba_small_misc.img $IMAGE_PATH/pcba_small_misc.img
	cp -va rkst/Image/pcba_whole_misc.img $IMAGE_PATH/pcba_whole_misc.img
	echo "done."
...
cp -v kernel/kernel.img $IMAGE_PATH/
cp -v kernel/resource.img $IMAGE_PATH/
cp -v u-boot/$U_BOOT_BIN rockdev
