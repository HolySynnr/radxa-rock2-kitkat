                ...
                mNumFramesWritten - numFramesPlayed;
                        ...
                        if(mFlags & FLAG_WFD_STREAMING)
                        {
                                ...
                                {       
                                        ...
                                        {
                                              if(sys_start_time + (mediaTimeUs - audio_start_timeUs) - pending_time< sys_time - 300000ll || (sys_time - last_adujst_time > 20000000ll && sys_start_time + (mediaTimeUs - audio_start_timeUs) -pending_time < sys_time -100000ll))//recalculate the mediaTimeUs.
                                                {
                                                        int retrtptxt;
                                                        if((retrtptxt = access("data/test/omx_rs_txt_file",0)) == 0)
                                                        {
                                                                ...
                                                                if(omx_rs_txt != NULL)
                                                                {
                                                                 ...
                                                                 fprintf(omx_rs_txt,"NuPlayer::Renderer::onDrainAudioQueue be fore delay 300ms start   %lld  %lld sys %lld %lld mediaTimeUs %lld last %lld delta %lld  %lld %lld\n" ,sys_start_time,audio_start_timeUs,last_adujst_time,sys_time,mediaTimeUs,last_timeUs,sys_time-sys_start_time-(mediaTimeUs - audio_start_timeUs) + pending_time, sys_time-sys_start_time-(mediaTimeUs - audio_start_timeUs),sys_time-last_adujst_time); 
                                                                        fflush(omx_rs_txt);
                                                                }
                                                        }
                            ALOGD("catchup:%lld:%lld:%lld:%lld:%lld:%d",sys_start_time,audio_start_timeUs,mediaTimeUs,sys_time,((sys_time - sys_start_time - (mediaTimeUs - audio_start_timeUs))/11)*11,mAudioQueue.size());
                                        ...
                                        else
                                        {
                                                ...
                                                if((retrtptxt = access("data/test/omx_rs_txt_file",0)) == 0)
                                                {
...

                                                        {
                                                           ...
                                                           fprintf(omx_rs_txt,"NuPlayer::Renderer::onDrainAudioQueue before drop sta rt %lld    %lld sys %lld %lld mediaTimeUs %lld last %lld delta %lld   %lld %lld\n" ,sys_start_time,audio_start_timeUs,last_adujst_time,sys_time ,mediaTimeUs,last_timeUs,sys_time-sys_start_time-(mediaTimeUs - audio_start_timeUs) + pending_time, sys_time-sys_start_time-(mediaTimeUs - audio_start_timeUs),sys_time-last_adujst_time);
                                                           fflush(omx_rs_txt);
                                                        }
                                                }
                                                {
                                                        ALOGD("discard late audio...%lld",mediaTimeUs);
...
void NuPlayer::Renderer::onDrainVideoQueue() {
    ...
    if(mVideoLateByUs + (audio_latency_time - 120000ll) < -50000 && (mFlags & FLAG_WFD_STREAMING)) {
    ...
    if (mFlags & FLAG_WFD_STREAMING) {
        entry->mNotifyConsumed->setInt32("render", 1);  //always render video
    } else {
