...
#include "bluray/include/HdmiManager.h"
...
public:
    ...
    ~TrackInfo()
    {
        if(mQueue != NULL)
        {
            ...
            {
                AVPacket* packet = mQueue->editItemAt(0);
                mQueue->removeAt(0);
                if(packet != NULL)
                {
...
public:
	...
	status_t        start();
	status_t        blurayStart(int createThread = 1);
	status_t        pause();
	bool            isPlaying();
    ...
    virtual void configMainVideo(bool configChange,int pid,Vector<BlurayVideoConfig*>* list,int begin);
    virtual void configExtendVideo(bool configChange,int pid,Vector<BlurayVideoConfig*>* list,int begin);
    ...
    /* 描叙:用于蓝光主视频文件播放位置的seek
    *  适用范围: 用于用户主动的seek动作和导航命名的seek
    *  参数: listPosition: 播放列表mVideolist的位置的索引
    *        offset: mVideolist[listPosition]对应m2ts的偏移，这只是一个粗略值
    *        time: 开始播放的时间(ms)
    */
    virtual void baseVideoSeek(int listPosition,int64_t offset,int64_t time);

    /* 描叙:用于蓝光3D MVC视频文件播放位置的seek
    *  适用范围: 用于用户主动的seek动作和导航命名的seek
    *  参数: listPosition: 播放列表mVideolist的位置的索引
    *        offset: mVideolist[listPosition]对应m2ts的偏移，这只是一个粗略值
    *        time: 开始播放的时间(ms)
    */
    virtual void extendVideoSeek(int listPosition,int64_t startOffset,int64_t time);
    ...
    */
    ...
    static void             vdecErrorCbk(void *me);
private:
     enum PlayerStatus{
...
//	static void*			startPlay(void* ptr);
	...
	static void 			decode(void* frame,int direct,int64_t pts,int64_t rawPts,DEC_TYPE dec_type,void *me);
    ...
    void                    getAudioCodecName(int type,char* buffer);
    void                    getVideoCodecName(int type,char* buffer);
    void                    closeAllFile();
...
/********add for test********/
    ...
    Vector<BlurayVideoConfig*>* 		mExtendVideolist;
	...
	int                                 mVideoPosition;
    int                                 mExtendVideoPosition;
	int                                 mAudioPosition;
    int                                 mSubtitlePosition;

    SeekCondition                       mVideoSeekCondition;
    SeekCondition                       mExtedndVideoSeekCondition;
    ...
    * 用于指示3D MVC文件
    */
    AVFormatContext *                   mExtendVideoContext;
    ...
    int                         mVideoThreadStatus;                 // 表明Main Video Packet Read 线程是否创建
  //  int                         mExtVideoThreadStatus;              // 表明Extend Video Packet Read 线程是否创建
    ...
    */
    int                        mVideoReadStatus;
    int                        mAudioReadStatus;
    int                        mSubtitleReadStatus;
    ...
    */
    ...
    /*
    * 用于控制HDMI(电视)的2D-3D的切换
    */

    HdmiManager*             mHdmiManager;

    int                      mVideoSurfaceZorder;
