    ...
    if (1) {
		...
		else
		{
	         ...
	         displayHeight = (int32_t)info.w;
                ...
                int fb_w = 1920;;
                int fb_h = 1080;
                if (property_get("sys.fb.cursize", pro_value, NULL) > 0) {
                      sscanf(pro_value, "%dx%d", &fb_w, &fb_h);
                }
                displayWidth = fb_w;
                displayHeight = fb_h;
        ...
        ALOGD("create a wfd surface with width %d height %d",displayWidth,displayHeight);
