...
public class DisplayOutputManager {
	...
	public void setMode(int display, int type, String mode) {
		...
		if ((type==DISPLAY_IFACE_HDMI) && "unknow".equals(mode)) {
	        ...
	        try {
				...
				tempString = reader.readLine();
				subString = tempString.substring(4, tempString.length());
				fps = Integer.parseInt(subString);
				mode = width+"x"+height+"p-"+fps;
				reader.close();
	        } catch (Exception e){
			}
           }
		...
		try {
			...
			if(display == MAIN_DISPLAY) {
                ...
                if (displaypolicy.equals("normal")) {
                  setWMDisplaySize(display, m, n) ;
                } else {
...
//			}
			...
			if (width >= 0 && height >= 0) {
				try {
					...
					if (mService != null) {
                              ...
                              SystemProperties.set("sys.fb.cursize", width+"x"+height);
				    }
	...
	public void setSat_con(int display, int sat)
	{
			
		try {
			mService.setSat_con(display, sat);
        } catch (Exception e) {
	...
	public void setHue(int display, int s1, int s2)
	{
			
		try {
			mService.setHue(display, s1,s2);
        } catch (Exception e) {
