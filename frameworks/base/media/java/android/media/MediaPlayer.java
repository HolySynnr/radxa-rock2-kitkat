...
import android.media.MediaTrackInfor;
...
public class MediaPlayer implements SubtitleController.Listener
{
	...
	private static final int MEDIAPLAYER_GET_PLAYING_AUDIO_TRACK_INDEX = 964;
	private static final int MEDIAPLAYER_GET_PLAYING_SUBTITLE_TRACK_INDEX = 965;
    ...
    public void setDataSource(Context context, Uri uri, Map<String, String> headers)
        ...
        if(scheme == null || scheme.equals("file")) {
			...
			boolean isBD = BlurayManager.isBDDirectory(pathString);
			if(isBD)
			{
	...
	public MediaTrackInfor[] getVideoTrack()
	{
		Parcel reply = getParcelParameter(MEDIAPLAYER_GET_VIDEO_TRACK);
		if(reply != null)
		{
			MediaTrackInfor trackInfo[] = reply.createTypedArray(MediaTrackInfor.CREATOR);
			if((trackInfo != null) && (trackInfo.length > 0))
			{
				Log.d(TAG,"Video Track Count = "+trackInfo.length);
			}
			return trackInfo;
		}
		return null;
	...
	public MediaTrackInfor[] getAudioTrack()
	{
		Parcel reply = getParcelParameter(MEDIAPLAYER_GET_ALL_AUDIO_TRACK);
		if(reply != null)
		{
			MediaTrackInfor trackInfo[] = reply.createTypedArray(MediaTrackInfor.CREATOR);
			if((trackInfo != null) && (trackInfo.length > 0))
	...
	public MediaTrackInfor[] getSubtitleTrack()
	{
		Parcel reply = getParcelParameter(MEDIAPLAYER_GET_ALL_SUBTITLE_TRACK);
		if(reply != null)
		{
			MediaTrackInfor trackInfo[] = reply.createTypedArray(MediaTrackInfor.CREATOR);
			if((trackInfo != null) && (trackInfo.length > 0))
	...
	{
		return getIntParameter(MEDIAPLAYER_GET_PLAYING_AUDIO_TRACK_INDEX);
	}

	public int getCurrentSubtitleTrackIndex()
	{
		return getIntParameter(MEDIAPLAYER_GET_PLAYING_SUBTITLE_TRACK_INDEX);
	}

	public boolean setAudioTrack(int index)
	{
		return setParameter(MEDIAPLAYER_SET_AUDIO_TRACK_INDEX,index);
	}

	public boolean setSubtitleTrack(int index)
	{
		return setParameter(MEDIAPLAYER_SET_SUBTITLE_TRACK_INDEX,index);
	}

	public int getSubtitleVisible()
	{
		return getIntParameter(MEDIAPLAYER_GET_SUBTITLE_VISIBLE);
	}

	public boolean setSubtitleVisible(int visible)
	{
		return setParameter(MEDIAPLAYER_SET_SUBTITLE_VISIBLE,visible);
	}

	public boolean setVideoSurfaceZorder(int layer)
	{
		return setParameter(MEDIAPLAYER_SET_VIDEO_SURFACEVIEW_ZORDER,layer);
	}
