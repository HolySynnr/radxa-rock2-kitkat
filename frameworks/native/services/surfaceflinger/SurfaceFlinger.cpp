        ...
        mDaltonize(false)
{
    ...
    property_get("persist.sys.display.policy", value, "0");
...
void SurfaceFlinger::init() {
	...
	if (property_get("video.use.overlay", property, "0") && atoi(property) == 1) {
...
void SurfaceFlinger::doComposition() {
    ...
    if (mDisplayPolicy == 1)
    {
      displayPolicy = ((mViewportChange==0 && mBootFinished==0)||(mViewportChange==1 && mBootFinished==1));
    }
    for (size_t dpy=0 ; dpy<mDisplays.size() ; dpy++) {
        const sp<DisplayDevice>& hw(mDisplays[dpy]);
         if (hw->canDraw() && displayPolicy){ 
           // transform the dirty region into this screen's coordinate space
            const Region dirtyRegion(hw->getDirtyRegion(repaintEverything));
    ...
    if (hasGlesComposition) {
    	...
    	if(id == 0 && mFakeWidth && mFakeHeight) {
			...
			if (property_get("persist.sys.overscan.main", property, NULL) > 0)
				sscanf(property, "overscan %d,%d,%d,%d", &overscan_left, &overscan_right, &overscan_top, &overscan_bottom);
			uint32_t xres, yres;
		...
		char mproperty[PROPERTY_VALUE_MAX];
		if (property_get("sys.use.3dmode", mproperty, NULL) )
		{
		    Rect mViewport;
		    Rect mFrame;
			...
			const sp<DisplayDevice>& hw1(mDisplays[0]);
			int dst_width  = hw1->getWidth();
    		int dst_height = hw1->getHeight();
			if(atoi(mproperty) == HDMI_3D_TOP_BOTTOM)
    			hw1->setProjection(0, mViewport, Rect(dst_width,dst_height/2));
			else if(atoi(mproperty) == HDMI_3D_SIDE_BY_SIDE_HALT)
				hw1->setProjection(0, mViewport, Rect(dst_width/2,dst_height));
			else
				hw1->setProjection(0, mViewport, mFrame);
        ...
        bool yswap)
{
	...
	char value[PROPERTY_VALUE_MAX];
	property_get("persist.sys.display.policy", value, "manual");
	if (!strcmp(value,"auto")){
		int fb_w;
		int fb_h;
		char property[PROPERTY_VALUE_MAX];
		if (property_get("sys.fb.cursize", property, NULL) > 0) {
			sscanf(property, "%dx%d", &fb_w, &fb_h);
		}
    // set-up our viewport
		engine.setViewportAndProjection(reqWidth*hw_w/fb_w, reqHeight*hw_h/fb_h, hw_w, hw_h, yswap);
	}else{
